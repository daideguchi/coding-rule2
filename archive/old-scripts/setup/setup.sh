...
## プロジェクト概要
- **名前**: AI開発支援プロジェクト
- **目的**: Cursor + Claude Code連携による効率的な開発環境の構築
- **言語**: 日本語メイン
...
## 開発フロー
1. **基本設定**: Cursor Rules適用
2. **環境準備**: Claude Code設定読み込み
3. **連携開発**: Cursor ↔ Claude Code同期
4. **品質確保**: 自動チェック・テスト

## 連携機能
- **同期スクリプト**: `./scripts/claude-cursor-sync.sh`
- **状況確認**: `./scripts/status-checker.sh`
- **AI組織**: `./ai-agents/manage.sh`

## 注意事項
- `.claude/`ディレクトリは.gitignoreに追加されており、ローカル設定のみ
- 設定は各環境で自動生成されるため、手動編集は非推奨
- 更新時は`./setup.sh`から再設定を推奨
EOF
...
- **プロジェクト名**: AI開発支援ツール
- **フェーズ**: 継続的改善・運用
- **主要技術**: Bash, JavaScript/TypeScript, AI組織システム

## 重要ディレクトリ
- `cursor-rules/`: Cursor IDE設定ルール
- `ai-agents/`: AI組織システム
- `scripts/`: 自動化スクリプト
- `logs/`: ログファイル管理

## 開発方針
1. **既存機能の保持**: 動作している機能を壊さない
2. **段階的改善**: 小さな改善を積み重ねる
3. **自動化重視**: 手動作業を最小限に抑える
4. **文書化**: 変更内容を適切に記録
